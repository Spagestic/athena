...
interface GazePos { normX: number; normY: number }
interface QOption { quad: Quad; label: string; text: string; correct: boolean }
interface Question { id: number; q: string; options: QOption[] }
...
const QUIZ: Question[] = [
    {
        ...
        options: [
            { quad: "tl", label: "A", text: "Ambition\n— your goal entering the role", correct: false },
            { quad: "tr", label: "B", text: "Action\n— the specific steps you personally took", correct: true },
            { quad: "bl", label: "C", text: "Analysis\n— your assessment of the situation", correct: false },
            { quad: "br", label: "D", text: "Achievement\n— your final result", correct: false },
        ],
    },
    {
        id: 2,
        ...
        options: [
            { quad: "tl", label: "A", text: "At the interviewer's face on screen", correct: false },
            { quad: "tr", label: "B", text: "At the camera lens itself", correct: true },
            { quad: "bl", label: "C", text: "Slightly down at your notes", correct: false },
            { quad: "br", label: "D", text: "At your own face in the preview", correct: false },
        ],
    },
    {
        id: 3,
        ...
        options: [
            { quad: "tl", label: "A", text: "38%\n— vocal tone only", correct: false },
            { quad: "tr", label: "B", text: "7%\n— words alone", correct: false },
            { quad: "bl", label: "C", text: "55%\n— body language and expressions", correct: true },
...
export default function EyeTraxStudy() {
    ...
    const sectionTimeoutRef = useRef<Record<string, ReturnType<typeof setTimeout> | null>>({});
    ...
    const startFrames = useCallback(() => {
        ...
        timerRef.current = setInterval(() => {
            const v = vidRef.current, ws = wsRef.current;
            const cnv = cnvRef.current, ctx = ctxRef.current;
    ...
    const connect = useCallback(async () => {
        try { await startCamera(); } catch { setStatus("error"); return; }
        setStatus("connecting");
        const ws = new WebSocket("ws://localhost:8766");
        wsRef.current = ws;
        ws.onopen = () => { setStatus("connected"); startFrames(); };
        ws.onmessage = (e) => {
            try {
        ...
        ws.onclose = () => { setStatus("idle"); stopFrames(); setGaze(null); };
    ...
    useEffect(() => {
        ...
        for (const [id, el] of Object.entries(sectionRefs.current)) {
            ...
            if (gx >= r.left && gx <= r.right && gy >= r.top && gy <= r.bottom) { hit = id; break; }
        }
        setGazeSection(hit);
    ...
    useEffect(() => {
        ...
        const quad: Quad = dispX < 0.5 ? (dispY < 0.5 ? "tl" : "bl") : (dispY < 0.5 ? "tr" : "br");
        ...
        const tick = () => {
            const elapsed = performance.now() - (dwellStartRef.current ?? performance.now());
        ...
        <div className="min-h-screen bg-background text-foreground font-mono select-none">
            {dispX !== null && dispY !== null && (
                <div
                    ...
                    style={{ left: `${dispX * 100}vw`, top: `${dispY * 100}vh`, transform: "translate(-50%,-50%)" }}
                >
                    <div className="w-6 h-6 rounded-full border-2 border-white opacity-90"
                        style={{ background: "rgba(34,197,94,0.55)", boxShadow: "0 0 14px 5px rgba(34,197,94,0.6)" }} />
                </div>
            )}
            ...
            <header className="border-b-2 border-foreground px-6 py-3 flex items-center justify-between bg-background z-50 sticky top-0">
                ...
                <div className="flex items-center gap-4">
                    ...
                    <span className={`text-[11px] font-bold tracking-widest ${STATUS_COLOR[status]}`}>
                        {status === "tracking" ? "● " : ""}{STATUS_LABEL[status]}
                    </span>
                    <button
                ...
                <div className="flex gap-0 min-h-[calc(100vh-57px)]">
                    <main className="flex-1 p-8 space-y-10 overflow-y-auto border-r-2 border-foreground">
                        <div className="max-w-2xl">
                            <p className="text-[10px] text-muted-foreground uppercase tracking-[0.3em] mb-1">Study Module — Interview Fundamentals</p>
                            <h1 className="text-2xl font-bold tracking-tight">Read the material below.</h1>
                            <p className="text-[12px] text-muted-foreground mt-2">
                                EyeTrax is tracking your gaze. Sections turn green once you have held your gaze on them.
                                Read all 4 to unlock the quiz.
                            </p>
                        </div>
                        {SECTIONS.map(s => {
                            ...
                            return (
                                <div
                                    ...
                                    style={{
                                        boxShadow: isRead ? "4px 4px 0 0 #22c55e" : isActive ? "4px 4px 0 0 var(--primary)" : "4px 4px 0 0 var(--foreground)",
                                        borderColor: isRead ? "#22c55e" : isActive ? "var(--primary)" : "var(--foreground)",
                                    }}
                                >
                                    <div className="flex items-start justify-between mb-4">
                                        <div className="flex items-center gap-3">
                                            <span className="text-[10px] font-bold px-2 py-0.5 border-2"
                                                style={{ borderColor: isRead ? "#22c55e" : "var(--foreground)", color: isRead ? "#22c55e" : "var(--foreground)" }}>
                                                {s.num}
                                            </span>
                                            <h2 className="text-sm font-bold tracking-widest">{s.title}</h2>
                                        </div>
                                        {isRead && <span className="text-[10px] font-bold text-green-400 tracking-widest">DONE</span>}
                                        {!isRead && isActive && <span className="text-[10px] text-primary tracking-widest animate-pulse">READING...</span>}
                                    </div>
                                    <div className="space-y-3">
                                        {s.lines.map((line, i) => (
                                            <p key={i} className="text-[13px] leading-relaxed text-muted-foreground">{line}</p>
                                        ))}
                                    </div>
                                </div>
                            );
                    ...
                    <aside className="w-72 p-6 flex flex-col gap-6 bg-card sticky top-[57px] h-[calc(100vh-57px)] overflow-y-auto">
                        <div>
                            <p className="text-[9px] text-muted-foreground uppercase tracking-widest mb-2">Camera Feed</p>
                            <div className="border-2 border-foreground overflow-hidden bg-black aspect-video relative"
                                style={{ boxShadow: "3px 3px 0 0 var(--foreground)" }}>
                                ...
                                {dispX !== null && dispY !== null && (
                                    <div className="absolute pointer-events-none"
                                        style={{ left: `${(1 - dispX) * 100}%`, top: `${dispY * 100}%`, transform: "translate(-50%,-50%)" }}>
                                        <div className="w-3 h-3 rounded-full border border-white" style={{ background: "rgba(34,197,94,0.7)" }} />
                                    </div>
                                )}
                            </div>
                        ...
                        <div className="border-2 border-foreground p-4" style={{ boxShadow: "3px 3px 0 0 var(--foreground)" }}>
                            <p className="text-[9px] text-muted-foreground uppercase tracking-widest mb-3">Reading Progress</p>
                            <div className="flex items-end gap-2 mb-3">
                                <span className="text-3xl font-bold" style={{ color: readPct === 100 ? "#22c55e" : "var(--primary)" }}>
                                    {readPct}%
                                </span>
                                <span className="text-[10px] text-muted-foreground mb-1">{readCount}/{SECTIONS.length}</span>
                            </div>
                            <div className="h-2 border border-foreground bg-muted overflow-hidden">
                                <div className="h-full transition-all duration-500"
                                    style={{ width: `${readPct}%`, background: readPct === 100 ? "#22c55e" : "var(--primary)" }} />
                            </div>
                            <div className="mt-3 space-y-1.5">
                                {SECTIONS.map(s => (
                                    <div key={s.id} className="flex items-center gap-2">
                                        <span className={`w-2 h-2 rounded-full flex-shrink-0 ${readSet.has(s.id) ? "bg-green-400" : gazeSection === s.id ? "bg-primary animate-pulse" : "bg-muted-foreground/30"}`} />
                                        <span className={`text-[9px] uppercase tracking-wider ${readSet.has(s.id) ? "text-green-400" : gazeSection === s.id ? "text-primary" : "text-muted-foreground"}`}>
                                            {s.num} {s.title}
                                        </span>
                                    </div>
                                ))}
                        ...
                        <div className="mt-auto space-y-2">
                            <button
                                ...
                                style={readCount === SECTIONS.length
                                    ? { background: "var(--primary)", color: "var(--primary-foreground)", boxShadow: "4px 4px 0 0 var(--foreground)" }
                                    : {}}
                            >
                                {readCount < SECTIONS.length ? `${SECTIONS.length - readCount} MORE TO READ` : "START QUIZ"}
                            </button>
                            <button
                                onClick={() => setPhase("quiz")}
                ...
                <div className="fixed inset-0 top-[57px] grid grid-cols-2 grid-rows-2 bg-background z-10">
                    <div className="absolute top-0 left-0 right-0 z-20 border-b-2 border-foreground bg-background px-8 py-4 flex flex-col items-center gap-1">
                        <p className="text-[9px] text-muted-foreground uppercase tracking-widest">
                            Question {qIndex + 1} of {QUIZ.length} — Look at your answer for {DWELL_MS / 1000}s to select
                        </p>
                        <p className="text-base font-bold tracking-tight text-center">{QUIZ[qIndex].q}</p>
                    </div>
                            ...
                            <div key={opt.quad}
                                ...
                                style={{
                                    background: isAnswered
                                        ? wasChosen ? opt.correct ? "rgba(34,197,94,0.12)" : "rgba(239,68,68,0.12)" : "transparent"
                                        : isSelected ? "rgba(234,88,12,0.07)" : "transparent",
                                    boxShadow: isSelected ? "inset 0 0 0 3px var(--primary)" : undefined,
                                }}
                            >
                                <span className="text-[10px] font-bold px-2 py-0.5 border-2 mb-4"
                                    style={{ borderColor: isSelected ? "var(--primary)" : "var(--muted-foreground)" }}>
                                    {opt.label}
                                </span>
                                <p className="text-sm font-bold text-center tracking-wide whitespace-pre-line">{opt.text}</p>
                                {isSelected && !isAnswered && (
                                    <div className="absolute bottom-4 left-1/2 -translate-x-1/2 w-20 h-2 border border-foreground bg-muted overflow-hidden">
                                        <div className="h-full bg-primary transition-none" style={{ width: `${dwellPct * 100}%` }} />
                                    </div>
                                )}
                                {isAnswered && wasChosen && (
                                    <div className={`absolute top-3 right-3 text-[10px] font-bold ${opt.correct ? "text-green-400" : "text-red-400"}`}>
                                        {opt.correct ? "CORRECT" : "WRONG"}
                                    </div>
                                )}
                                {isAnswered && !wasChosen && opt.correct && (
                                    <div className="absolute top-3 right-3 text-[10px] text-green-400/60">CORRECT ANSWER</div>
                                )}
                                <span className="absolute top-2 left-2 text-[9px] text-muted-foreground/30 uppercase">{opt.quad}</span>
                            </div>
                        );
                    })}
                </div>
            ...
            {phase === "result" && (
                <div className="min-h-[calc(100vh-57px)] flex items-center justify-center p-8">
                    <div className="border-2 border-foreground p-10 max-w-lg w-full text-center"
                        style={{ boxShadow: "6px 6px 0 0 var(--foreground)" }}>
                        <p className="text-[10px] text-muted-foreground uppercase tracking-[0.3em] mb-6">Quiz Complete</p>
                        <div className="text-7xl font-bold mb-2"
                            style={{ color: score === QUIZ.length ? "#22c55e" : score >= QUIZ.length / 2 ? "var(--primary)" : "#ef4444" }}>
                            {score}/{QUIZ.length}
                        </div>
                        <p className="text-sm text-muted-foreground mb-8 tracking-wide">
                            {score === QUIZ.length ? "PERFECT — You read carefully."
                                : score >= QUIZ.length / 2 ? "GOOD — Review the missed sections."
                            ...
                            {QUIZ.map(q => {
                                ...
                                return (
                                    <div key={q.id} className="space-y-1">
                                        ...
                                        <p className={`text-[11px] ${correct ? "text-green-400" : "text-red-400"}`}>
                                            {correct ? "CORRECT" : "WRONG"}: {sel?.text.split("\n")[0] ?? "—"}
                                        </p>
                                        {!correct && (
                                            <p className="text-[10px] text-muted-foreground">
                                                Answer: {q.options.find(o => o.correct)?.text.split("\n")[0]}
                                            </p>
                                        )}
                                    </div>
                                );
                        ...
                        <div className="border-t-2 border-foreground pt-6 mb-8">
                            <p className="text-[9px] text-muted-foreground uppercase tracking-widest mb-3">Sections Read</p>
                            <div className="flex justify-center gap-2">
                                {SECTIONS.map(s => (
                                    <div key={s.id} className={`text-[9px] px-2 py-0.5 border ${readSet.has(s.id) ? "border-green-400 text-green-400" : "border-muted-foreground/30 text-muted-foreground"}`}>
                                        {s.num}
                                    </div>
                                ))}
                            </div>
                        ...
                        <div className="flex gap-3 justify-center">
                            <button
                                onClick={() => {
                                    setPhase("study"); setQIndex(0); setScore(0); setAnswered({});
                                    setReadSet(new Set()); sectionTimeoutRef.current = {};
                                    dwellQuadRef.current = null; setDwellQuad(null); setDwellPct(0);
                                }}
                            ...
                            <button
                                onClick={() => {
                                    setPhase("quiz"); setQIndex(0); setScore(0); setAnswered({});
                                    dwellQuadRef.current = null; setDwellQuad(null); setDwellPct(0);
                                }}
                                className="px-5 py-2.5 text-[11px] font-bold uppercase tracking-widest border-2 border-foreground hover:bg-foreground hover:text-background transition-colors"
                                style={{ boxShadow: "3px 3px 0 0 var(--foreground)" }}
