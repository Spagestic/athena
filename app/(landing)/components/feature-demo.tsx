...
const features = [
  {
    ...
    specs: ["FLASHCARDS", "PRACTICE TESTS", "MIND MAPS", "TUTOR", "NOTES", "PODCASTS"],
  },
  {
    id: "flashcards",
    label: "FLASHCARDS",
    ...
    specs: ["COLUMN VIEW", "TOGGLE + LIST", "TYPING MODE", "LEARN MODE", "AI GENERATION"],
  },
  {
    id: "track",
    label: "TRACK",
    ...
    specs: ["VOICE TUTORING", "AUTO PAGE-FLIP", "COMPREHENSION CHECKS", "32+ LANGUAGES", "VISUAL NARRATION"],
  },
  {
    id: "notes",
    label: "NOTES",
    ...
    specs: ["BLOCK EDITOR", "AI AUTOCOMPLETE", "SOURCE CITATIONS", "RICH FORMATTING", "EXPORT"],
  },
  {
    id: "mindmap",
    label: "MINDMAP",
    ...
    specs: ["AUTO-GENERATION", "INTERACTIVE NODES", "CONCEPT LINKING", "ZOOM + PAN", "EXPORT"],
  },
  {
    id: "practice-test",
    label: "PRACTICE TEST",
    ...
    specs: ["MCQ GENERATION", "SHORT ANSWER", "AUTO-MARKING", "DIFFICULTY SCALING", "CITED FEEDBACK"],
  },
]
...
function WireframeStudio() {
  return (
    <div className="font-mono text-[11px] bg-[#0a0a0a] text-white w-full h-full flex flex-col border border-white/20" style={{ minHeight: 340 }}>
      {/* header */}
      <div className="flex items-center justify-between px-3 py-2 border-b border-white/20 bg-[#111]">
        <span className="tracking-widest text-white/80 text-[10px]">ATHENA STUDIO</span>
      ...
      <div className="grid grid-cols-3 gap-1.5 p-3 border-b border-white/10">
        {[
          ...
          ["Practice Test", ""], ["Tutor", "✎"], ["Track", "✎"],
          ["Infographic", ""], ["Podcast", "✎"], ["More", "···"],
        ].map(([label, icon]) => (
          <div key={label} className="flex items-center gap-1 px-1.5 py-1 border border-white/20 bg-white/5 text-[9px] text-white/70">
            <span className="w-1.5 h-1.5 rounded-full bg-white/30 flex-shrink-0" />
            <span className="truncate">{label}</span>
            {icon && <span className="ml-auto text-white/40 text-[9px]">{icon}</span>}
          </div>
        ))}
      </div>
      {/* bottom item row */}
      <div className="flex items-center justify-between px-3 py-2 border-b border-white/10">
        ...
        <span className="text-[9px] border border-white/20 px-2 py-0.5 text-white/60">Learn All</span>
      </div>
      <div className="px-3 py-2.5 flex-1">
        <p className="text-white/80 mb-1">Flashcards: Head &amp; Neck Anatomy</p>
        <p className="text-[9px] text-white/40">FLASHCARDS · 131 items · 1 day ago</p>
      </div>
...
function WireframeFlashcards() {
  return (
    <div className="font-mono text-[11px] bg-[#0a0a0a] text-white w-full h-full flex flex-col border border-white/20" style={{ minHeight: 340 }}>
      <div className="flex items-center justify-between px-3 py-2 border-b border-white/20 bg-[#111]">
        <span className="tracking-widest text-white/80 text-[10px]">ATHENA.FLASHCARDS</span>
        <span className="flex items-center gap-1.5 text-[10px]">
          <span className="w-1.5 h-1.5 rounded-full bg-[#ea580c] animate-pulse" />
      ...
      <div className="flex-1 flex flex-col justify-center px-5 py-4 gap-4">
        <span className="text-[9px] text-white/40 tracking-widest">CARD 12 OF 48</span>
        <div className="border border-white/20 p-4 bg-[#111]">
          <div className="border-b border-white/10 pb-3 mb-3 text-white/90 leading-snug">
            What is the mechanism of action<br />of beta-blockers?
          </div>
        ...
        <div className="grid grid-cols-4 gap-1.5 mt-1">
          {["← AGAIN", "HARD", "GOOD", "EASY →"].map((label) => (
            <button key={label} className={`border py-1.5 text-[9px] tracking-wide transition-colors ${label === "GOOD" ? "border-[#ea580c] text-[#ea580c]" : "border-white/20 text-white/50 hover:border-white/40"}`}>
              {label}
            </button>
          ))}
        </div>
...
function WireframeNotes() {
  return (
    <div className="font-mono text-[11px] bg-[#0a0a0a] text-white w-full h-full flex flex-col border border-white/20" style={{ minHeight: 340 }}>
      <div className="flex items-center justify-between px-3 py-2 border-b border-white/20 bg-[#111]">
        <span className="tracking-widest text-white/80 text-[10px]">ATHENA.NOTES</span>
      ...
      <div className="flex-1 px-4 py-3 space-y-2 leading-relaxed">
        ...
        <p className="text-white/80"><span className="text-white font-bold">**Dyspnoea**</span> — unexpected awareness</p>
        <p className="text-white/80">of breathing. <span className="text-[#ea580c]">[1]</span></p>
        <p className="text-white/40 text-[9px]">&nbsp;</p>
        ...
        <div className="border-t border-white/10 pt-2">
          <p className="text-[9px] text-white/40">SOURCE: respiratory.pdf · pg.4 <span className="text-[#ea580c]">[1]</span></p>
        </div>
      </div>
    </div>
  )
...
function WireframePracticeTest() {
  return (
    <div className="font-mono text-[11px] bg-[#0a0a0a] text-white w-full h-full flex flex-col border border-white/20" style={{ minHeight: 340 }}>
      <div className="flex items-center justify-between px-3 py-2 border-b border-white/20 bg-[#111]">
        <span className="tracking-widest text-white/80 text-[10px]">ATHENA.TEST</span>
        <span className="text-[10px] text-white/50">Q 3 OF 12</span>
      </div>
      <div className="flex-1 px-4 py-3 flex flex-col gap-3">
        <p className="text-white/90 leading-snug">A 65-year-old presents with<br />orthopnoea and PND. Most likely<br />diagnosis?</p>
        <div className="border-t border-white/10" />
        <div className="space-y-2">
          {[
            { letter: "A", label: "Asthma", selected: false },
            { letter: "B", label: "Cardiac failure", selected: true, cite: "[1]" },
            { letter: "C", label: "COPD", selected: false },
            { letter: "D", label: "Pulmonary embolism", selected: false },
          ].map(({ letter, label, selected, cite }) => (
            <div key={letter} className={`flex items-center justify-between px-2 py-1.5 border ${selected ? "border-[#ea580c] bg-[#ea580c]/10" : "border-white/10"}`}>
              <span className={`flex items-center gap-2 ${selected ? "text-white" : "text-white/50"}`}>
                <span>{selected ? "●" : "○"}</span>
                <span>{letter}. {label}</span>
              </span>
              {cite && <span className="text-[9px] text-[#ea580c]">← {cite}</span>}
            </div>
          ))}
        </div>
        <div className="mt-auto pt-2 border-t border-white/10 flex items-center gap-3">
          ...
          <div className="flex-1 flex gap-0.5">
            {[1, 1, 1, 1, 0, 0, 0, 0].map((filled, i) => (
              <div key={i} className={`h-1.5 flex-1 ${filled ? "bg-[#ea580c]" : "bg-white/10"}`} />
            ))}
          </div>
...
function WireframeMindMap() {
  return (
    <div className="font-mono text-[11px] bg-[#0a0a0a] text-white w-full h-full flex flex-col border border-white/20" style={{ minHeight: 340 }}>
      <div className="flex items-center justify-between px-3 py-2 border-b border-white/20 bg-[#111]">
        <span className="tracking-widest text-white/80 text-[10px]">ATHENA.MINDMAP</span>
      ...
      <div className="flex-1 relative p-4 overflow-hidden">
        ...
        <svg className="w-full h-full" viewBox="0 0 320 240" fill="none">
          {/* center node */}
          <rect x="110" y="100" width="100" height="28" rx="2" fill="#1a1a1a" stroke="rgba(255,255,255,0.3)" strokeWidth="1" />
          <text x="160" y="119" textAnchor="middle" fill="rgba(255,255,255,0.9)" fontSize="9" fontFamily="monospace">Respiratory</text>
          {/* branches */}
          {[
            ...
            <g key={label}>
              <line x1="160" y1="114" x2={x + 50} y2={y + 12} stroke="rgba(234,88,12,0.5)" strokeWidth="1" />
              <rect x={x} y={y} width="100" height="24" rx="2" fill="#111" stroke="rgba(255,255,255,0.15)" strokeWidth="1" />
              <text x={x + 50} y={y + 15} textAnchor="middle" fill="rgba(255,255,255,0.6)" fontSize="8" fontFamily="monospace">{label}</text>
            </g>
          ))}
          {/* leaf nodes */}
          <rect x="0" y="30" width="70" height="18" rx="2" fill="#0d0d0d" stroke="rgba(255,255,255,0.08)" strokeWidth="1" />
          <text x="35" y="42" textAnchor="middle" fill="rgba(255,255,255,0.35)" fontSize="7" fontFamily="monospace">Awareness</text>
          <line x1="20" y1="60" x2="35" y2="48" stroke="rgba(255,255,255,0.1)" strokeWidth="1" />
        </svg>
        {/* labels */}
        <div className="absolute bottom-3 left-3 right-3 flex items-center gap-2">
          <span className="text-[9px] text-white/30">respiratory.pdf</span>
          <span className="ml-auto text-[9px] border border-white/10 px-2 py-0.5 text-white/40">Export ↗</span>
        </div>
      </div>
    </div>
  )
...
function WireframeTutor() {
  return (
    <div className="font-mono text-[11px] bg-[#0a0a0a] text-white w-full h-full flex flex-col border border-white/20" style={{ minHeight: 340 }}>
      <div className="flex items-center justify-between px-3 py-2 border-b border-white/20 bg-[#111]">
        <span className="tracking-widest text-white/80 text-[10px]">ATHENA.TUTOR</span>
      ...
      <div className="flex-1 flex flex-col gap-3 px-4 py-3">
        ...
        <div className="flex items-start gap-2">
          <div className="w-6 h-6 bg-[#ea580c]/20 border border-[#ea580c]/40 flex items-center justify-center flex-shrink-0 text-[8px] text-[#ea580c]">AI</div>
          <div className="bg-[#111] border border-white/10 px-2.5 py-2 text-white/80 leading-snug max-w-[85%]">
            Let&apos;s look at <span className="text-[#ea580c]">dyspnoea</span>. It&apos;s the unexpected<br />awareness of breathing — page 4 of your PDF.<br />
            <span className="text-white/40 text-[9px] mt-1 block">pg. 4 · respiratory.pdf</span>
          </div>
        </div>
        {/* user message */}
        <div className="flex items-start gap-2 flex-row-reverse">
          <div className="w-6 h-6 bg-white/10 border border-white/20 flex items-center justify-center flex-shrink-0 text-[8px] text-white/50">ME</div>
          <div className="bg-[#181818] border border-white/10 px-2.5 py-2 text-white/70 max-w-[85%]">
            What&apos;s the difference between cardiac<br />and respiratory dyspnoea?
          </div>
        </div>
        {/* AI typing */}
        <div className="flex items-start gap-2">
    ...
    <section className="border-t-2 border-foreground">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_1.6fr] border-b-2 border-foreground">
        ...
        <AnimatePresence mode="wait">
          ...
          >
            {/* Wireframe area */}
            <div className="relative flex-1 bg-[#0a0a0a] overflow-hidden" style={{ minHeight: "340px" }}>
              {/* Corner brackets */}
