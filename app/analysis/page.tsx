...
export default function AnalysisDashboard() {
  ...
  const eyetraxIntervalRef = useRef<ReturnType<typeof setInterval> | null>(
    null,
  );
  ...
  const [energyComponents, setEnergyComponents] = useState<EnergyComponents>({
    energy: 0,
    eye: 0,
    expr: 0,
    alertness: 0,
  ...
  const [eyetraxStatus, setEyetraxStatus] =
    useState<EyetraxStatus>("disconnected");
    ...
    const yawnColor =
      yawnVal > 50
        ...
        : yawnVal > 25
          ...
          : "text-chart-4";
    return {
      eye: `${Math.round(eyeVal)}%`,
      eyeSub: eyeOk
        ? "GOOD CONTACT"
        : eyeVal < 50
          ? "LOOK AT CAMERA"
          : "STARING",
      eyeColor: eyeOk ? "text-chart-4" : "text-destructive",
      stab: stabOk ? "STABLE" : "MOVING",
      stabSub: `YAW ${yaw.toFixed(0)}\u00b0 / PITCH ${pitch.toFixed(0)}\u00b0`,
      stabColor: stabOk ? "text-chart-4" : "text-chart-5",
      expr: `${Math.round(exprVal)}%`,
      exprSub: exprVal > 40 ? "EXPRESSIVE" : "FLAT",
      yawn: yawnVal > 50 ? "YAWNING" : yawnVal > 25 ? "TIRED?" : "ALERT",
      yawnSub: `${Math.round(yawnVal)}% MOUTH OPEN`,
      yawnColor,
      energy: `${energyVal}`,
      energySub:
        energyVal > 70
  ...
  const audioKpis: AudioKpis = (() => {
    ...
    const fillersTotal = fillerData.reduce(
      (s, d) => s + Number(d.inTurn || 0),
      0,
    );
    ...
    return {
      words: wordsTotal,
      wordsSub: `${wpmData.length} TURNS`,
      fillers: fillersTotal,
      fillersSub: fillersTotal > 5 ? "TOO MANY" : "ACCEPTABLE",
      volume: `${volVal.toFixed(0)}dB`,
      volumeSub: volVal > -20 ? "LOUD" : volVal > -40 ? "GOOD" : "QUIET",
      pitch: `${pitchVal.toFixed(0)}Hz`,
      pitchSub: pitchVal > 200 ? "HIGH" : pitchVal > 100 ? "NORMAL" : "LOW",
      mono,
      turns: wpmData.length,
      turnsSub: `AVG ${wpmData.length > 0 ? Math.round(wordsTotal / wpmData.length) : 0} WPM`,
      stutters: audioLog.filter(
        (e) => e.type === "warn" && e.text.toLowerCase().includes("repeat"),
      ).length,
    };
  })();
  ...
  const dots = {
    ...
    gaze:
      eyetraxStatus === "tracking"
        ? ("on" as const)
        : eyetraxStatus !== "disconnected"
          ? ("warn" as const)
          : ("off" as const),
    ...
    (results: {
      ...
      if (nowMs - fpsLast.current >= 1000) {
        if (fpsBadgeRef.current)
          fpsBadgeRef.current.textContent = `${fpsFrames.current} FPS`;
        fpsFrames.current = 0;
      ...
      if (vw) {
        cnv.width = vw.clientWidth;
        cnv.height = vw.clientHeight;
      }
      ...
      const eyeContactBase = clamp(
        (1 - Math.max(0, hOff - 0.08) * 4) * 100,
        0,
        100,
      );
      ...
      if (eyeContactBufferRef.current.length > 40)
      ...
      const smileScore = clamp(((smileRatio - 0.5) / 0.3) * 100, 0, 100);
      ...
      const liftScore = clamp(((cornerLift + 0.05) / 0.1) * 100, 0, 100);
      // Brow raise (bonus engagement signal)
      const browLift =
        avg(BROW_L.map((i) => lm[i])).y - avg(L_EYE.map((i) => lm[i])).y;
      const browLiftR =
        avg(BROW_R.map((i) => lm[i])).y - avg(R_EYE.map((i) => lm[i])).y;
      const browRaise = clamp(Math.abs(browLift + browLiftR) * 300, 0, 25);
      // 60% mouth width, 25% corner lift, 15% brow — no smile → low score
      const exprScore = clamp(
        smileScore * 0.6 + liftScore * 0.25 + browRaise,
        0,
        100,
      );
      ...
      const eyeContactAvg =
        buf.length > 0 ? buf.reduce((a, b) => a + b, 0) / buf.length : 0;
      const yawnBuf = yawnBufferRef.current;
      const yawnAvg =
        yawnBuf.length > 0
          ? yawnBuf.reduce((a, b) => a + b, 0) / yawnBuf.length
          : 0;
      ...
      const headStability = clamp(
        100 - (Math.abs(yaw) + Math.abs(pitch)) * 5,
        0,
        100,
      );
      const alertness = 100 - yawnAvg;
      const energyScore = Math.round(
        (eyeContactAvg + exprScore + alertness + headStability) / 4,
      );

      setEyeData((prev) =>
        push(prev, { label: t, value: Math.round(eyeContactAvg) }),
      );
      setExprData((prev) =>
        push(prev, {
          label: t,
          expr: Math.round(exprScore),
          brow: Math.round(browRaise),
        }),
      );
      setHeadData((prev) =>
        push(prev, {
      ...
      setYawnData((prev) =>
        push(prev, { label: t, value: Math.round(yawnAvg) }),
      );
      setEnergyData((prev) => push(prev, { label: t, value: energyScore }));
      setEnergyComponents({
        energy: energyScore,
        eye: Math.round(eyeContactAvg),
        expr: Math.round(exprScore),
        ...
        setVideoLog((prev) =>
          push(
            prev,
            {
              ts: t,
              type: "warn",
              text: `Low eye contact: ${Math.round(eyeContactAvg)}%`,
            },
            200,
          ),
        );
        ...
        setVideoLog((prev) =>
          push(
            prev,
            {
              ts: t,
              type: "good",
              text: `Good eye contact: ${Math.round(eyeContactAvg)}%`,
            },
            200,
          ),
        );
        ...
        setVideoLog((prev) =>
          push(
            prev,
            {
              ts: t,
              type: "warn",
              text: `Head off-axis: yaw ${yaw.toFixed(0)}\u00b0 pitch ${pitch.toFixed(0)}\u00b0`,
            },
            200,
          ),
        );
        ...
        setVideoLog((prev) =>
          push(
            prev,
            {
              ts: t,
              type: "warn",
              text: `Yawn detected \u2014 energy may be low (${Math.round(yawnAvg)}%)`,
            },
            200,
          ),
        );
      ...
      if (energyScore > 72) {
        setVideoLog((prev) =>
          push(
            prev,
            { ts: t, type: "good", text: `High energy: ${energyScore}/100` },
            200,
          ),
        );
      }
  ...
  /* auto-scroll logs */
  useEffect(() => {
    vLogRef.current?.scrollTo(0, vLogRef.current.scrollHeight);
  }, [videoLog]);
  useEffect(() => {
    aLogRef.current?.scrollTo(0, aLogRef.current.scrollHeight);
  }, [audioLog]);
  useEffect(() => {
    fLogRef.current?.scrollTo(0, fLogRef.current.scrollHeight);
  }, [fillerLog]);

  /* ─── Audio handlers ─── */
  const handleAudio = useCallback(
    (data: { volume?: number; pitch?: number; pitch_std?: number }) => {
      ...
      if (data.pitch !== undefined) {
        setPitchData((prev) =>
          push(prev, { label: t, hz: data.pitch!, std: data.pitch_std || 0 }),
        );
      }
    },
    [],
  ...
  const handleTranscript = useCallback((text: string) => {
    ...
    for (const f of FILLERS) {
      ...
      if (matches) {
        ...
        for (const m of matches) {
          setFillerLog((prev) =>
            push(prev, { ts: t, type: "filler", text: `"${m}" detected` }, 200),
          );
        }
      }
    }
    setFillerData((prev) => {
      const cum =
        prev.reduce((s, d) => s + Number(d.inTurn || 0), 0) + turnFillers;
      return push(prev, {
        label: `T${prev.length + 1}`,
        inTurn: turnFillers,
        cumulative: cum,
    ...
    setAudioLog((prev) =>
      push(
        ...
        {
          ts: t,
          type: turnFillers > 0 ? "warn" : "good",
          text: `${words.length} words, ${wpm} wpm${turnFillers > 0 ? `, ${turnFillers} fillers` : ""}`,
        },
        200,
      ),
    );
    ...
    for (let i = 1; i < words.length; i++) {
      if (words[i] === words[i - 1]) {
        setAudioLog((prev) =>
          push(
            prev,
            { ts: t, type: "warn", text: `Repeated word: "${words[i]}"` },
            200,
          ),
        );
      }
  ...
  useEffect(() => {
    ...
    const loop = () => {
      ...
      const fm = faceMeshRef.current as {
    ...
    loop();
    return () => {
      clearTimeout(faceReqRef.current);
    };
  }, [running]);
  ...
  const initFaceMesh = useCallback(() => {
    ...
    const FaceMeshClass = w.FaceMesh as new (opts: {
  ...
  const connectEyeTrax = useCallback(() => {
    ...
    ws.onclose = (ev) => {
      ...
      eyetraxWsRef.current = null;
    };
    ws.onerror = (ev) => {
      console.error(
        "[EyeTrax] WebSocket error — is eyetrax_server.py running?",
        ev,
      );
      setEyetraxStatus("disconnected");
    };
  ...
  const calibrateEyeTrax = useCallback(() => {
    if (eyetraxWsRef.current?.readyState === 1) {
      eyetraxWsRef.current.send(
        JSON.stringify({ type: "calibrate", method: "9-point" }),
      );
    ...
    <div className="min-h-screen flex flex-col bg-background text-foreground font-mono">
      ...
      <div className="flex items-center justify-between px-6 py-3 bg-card border-b-2 border-foreground">
        ...
        <div className="flex items-center gap-4">
          ...
          <span
            className={`px-3 py-1 text-[10px] font-bold uppercase tracking-widest border-2 ${liveClasses}`}
          >
      ...
      <div className="flex flex-wrap gap-4 px-6 py-2 bg-card border-b-2 border-foreground text-[10px] font-mono uppercase tracking-widest text-muted-foreground">
        <span>
          <StatusDot state={dots.ws} />
          WEBSOCKET
        </span>
        <span>
          <StatusDot state={dots.cam} />
          CAMERA
        </span>
        <span>
          <StatusDot state={dots.mic} />
          MICROPHONE
        </span>
        <span>
          <StatusDot state={dots.gaze} />
          EYETRAX{" "}
          {eyetraxStatus !== "disconnected" &&
            `(${eyetraxStatus.toUpperCase()})`}
        </span>
        <span>
          <StatusDot state={dots.face} />
          FACE MESH
        </span>
      </div>
