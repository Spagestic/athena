...
export function StatusDot({ state }: { state: "on" | "off" | "warn" }) {
  ...
  return (
    <span
      className={`inline-block w-2 h-2 mr-1.5 border border-foreground ${bg}`}
    />
  );
}
...
export function KpiCard({
  ...
  return (
    <div className="border-2 border-foreground bg-card p-3 shadow-md">
      ...
      <p
        className={`text-3xl font-bold font-mono tabular-nums leading-none mt-1 ${colorClass}`}
      >
        {value}
      </p>
  ...
  return (
    <div className="border-2 border-foreground bg-card p-4 shadow-md">
      <div className="flex items-center justify-between mb-2">
        ...
        {warn && (
          <span className="text-[9px] font-mono font-bold uppercase tracking-widest text-destructive animate-pulse">
            {warn}
          </span>
        )}
      </div>
...
export const badgeCls = (t: string) => {
  switch (t) {
    case "good":
      return "bg-chart-4/20 text-chart-4 border border-chart-4";
    case "warn":
      return "bg-chart-5/20 text-chart-5 border border-chart-5";
    case "bad":
...
export function LogLine({ entry }: { entry: LogEntry }) {
  return (
    <div className="flex gap-2 items-baseline py-1 border-b border-border text-[11px] font-mono">
      ...
      <span
        className={`px-1.5 py-0.5 text-[8px] font-bold uppercase tracking-widest ${badgeCls(entry.type)}`}
      >
